...
class app:
    ...
    def parse_args(self):
        ...
        resource_map_parser.add_argument(
            "--eks-kms", help="Whether or not a KMS key generated by the cloudformation stack is configured", default=False, action="store_true"
        )
        resource_map_parser.add_argument(
            "--efs-backups",
    ...
    def generate_resource_map(
        ...
        if eks_kms:
            optional_resources.append(resources["eks_kms"])
    ...
    def get_imports(self):
        ...
        if self.args.resource_map:
            ...
            resource_map = self.generate_resource_map(
                ...
                eks_kms=not bool(self.cdkconfig["eks"]["secrets_encryption_key_arn"]),
            )
