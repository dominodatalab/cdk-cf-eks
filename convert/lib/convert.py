...
class app:
    ...
    def parse_args(self):
        ...
        create_tfvars_parser = subparsers.add_parser(name="create-tfvars", help="Generate tfvars", parents=[common_parser])
        ...
        resource_map_parser = subparsers.add_parser(name="resource-map", help="Create resource map for customization/debugging of get-imports command (optional step)")
        resource_map_parser.add_argument("--availability-zones", help="Availability zone count", default=3, type=int)
        resource_map_parser.add_argument("--route53", help="Whether or not to import route53 zones", default=False, action="store_true")
        resource_map_parser.add_argument("--efs-backups", help="Whether or not to import efs backup vault", default=True, action=argparse.BooleanOptionalAction)
        resource_map_parser.set_defaults(command=self.resource_map)

        import_parser = subparsers.add_parser(name="get-imports", help="Get terraform import commands", parents=[common_parser])
        import_parser.add_argument("--availability-zones", help="Availability zone count override. Default: autodetect from cdk config; no effect when using a resource map", default=None, type=int)
        ...
        clean_stack_parser = subparsers.add_parser(name="clean-stack", help="Clean stack something something", parents=[common_parser])
        clean_stack_parser.add_argument("--delete", help="Delete unneeded stack items", default=False, action="store_true")
        clean_stack_parser.add_argument("--all-staged-resources", help="Print all potentially deleteable resources from stack, without checking if they exist", default=False, action="store_true")
        clean_stack_parser.add_argument("--include-types", help=f"Only processes specified categories: {clean_categories}", type=lambda s: [x.strip() for x in s.split(',')])
        clean_stack_parser.add_argument("--resource-file", help="Load resources from yaml file generated via --print-stack --verbose --yaml; useful if stack is gone")
        clean_stack_parser.add_argument("--remove-security-group-references", help="Remove rules referencing security groups to be deleted. Otherwise, prints offending groups for manual deletion.", default=False, action="store_true")
        ...
        print_stack_parser.add_argument("--sub-stack", help=f"Sub-stacks to print, can be any of {stack_map.values()}", default=None)
        print_stack_parser.add_argument("--verbose", help="Print verbose stack info", default=False, action="store_true")
    ...
    def resource_map(self):
        resource_map = self.generate_resource_map(self.args.availability_zones, self.args.efs_backups, self.args.route53)
        print(yaml.safe_dump(resource_map))

    def get_imports(self):
        self.setup()
        cdkconfig = yaml.safe_load([o["OutputValue"] for o in self.cf.describe_stacks(StackName=self.stack_name)["Stacks"][0]["Outputs"] if o["OutputKey"] == "cdkconfig"][0])
        ...
        for map_stack, items in resource_map.items():
            ...
            for item in items:
                ...
                if value := item.get("value"):
                    ...
                    resource_id = (
                        f"{resources[t(assoc['subnet'])]}/{resources[t(assoc['route_table'])]}"
                    )
    ...
    def create_tfvars(self):
        ...
        tfvars = {
            ...
            "vpc_id": ckdconfig["vpc"]["id"] if not cdkconfig["vpc"]["create"] else self.stacks["vpc_stack"]["resources"]["VPC"],
            ...
            "route53_hosted_zone_name": reduce(lambda cfg, k: cfg[k] if k in cfg else [""], ["install", "overrides", "external_dns", "domain_filters"], cdkconfig)[0],
            "efs_backups": cdkconfig["efs"]["backup"]["enable"],
    ...
    def clean_stack(self):
        ...
        lambda_safe = [cdk_ids.lambda_function.value, cdk_ids.iam_role.value, cdk_ids.lambda_layerversion.value, cdk_ids.stepfunctions_statemachine.value]
        nukey = {
            "efs_stack": {
            ...
            "eks_stack": {
                f"(snapshot|chibipug7501ebscsi|{self.stack_name}DominoEcrRestricted|autoscaler)": [cdk_ids.iam_policy.value],
            ...
            "vpc_stack": {
                ...
                "bastion": [cdk_ids.instance.value, cdk_ids.instance_profile.value, cdk_ids.iam_role.value, cdk_ids.eip.value],
        ...
        def get_nukes(stack_name, stack_resources):
            for k, v in stack_resources.items():
                for nukey_regex in nukey[stack_name].keys():
                    if re.match(nukey_regex, k) and v['ResourceType'] in nukey[stack_name][nukey_regex] and v['ResourceType'] in nuke_queue:
        ...
        nuke(region=self.region, verbose=self.args.verbose, delete=self.args.delete).nuke(nuke_queue, self.args.remove_security_group_references)

    def delete_stack(self):
        self.setup()

        cf_tf_output = json.loads(run(["terraform", "output", "-json"], cwd="cloudformation-only", capture_output=True).stdout)
        if not (cf_only_role := cf_tf_output.get("cloudformation_only_role")) or not re.match("arn:.*:iam:.*role/cloudformation-only", cf_only_role["value"]):
